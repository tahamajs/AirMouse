...
class OrientationMonitorService : Service(), SensorEventListener {
    ...
    private var lastRestingState = false
    private val serviceScope = CoroutineScope(Dispatchers.IO + SupervisorJob())
    private var gravity = FloatArray(3) // low-pass filter
    ...
    companion object {
        ...
        private const val FLAT_THRESHOLD = 0.2f // g-force tolerance (0.2 = ~11 degrees tilt)
    }
    ...
    override fun onSensorChanged(event: SensorEvent) {
        ...
        // Calculate magnitude of gravity on X and Y axes
        val xyMagnitude = Math.hypot(gravity[0].toDouble(), gravity[1].toDouble())
        // If phone is flat, gravity should be mostly on Z, so XY magnitude is small
        val isFlat = xyMagnitude < FLAT_THRESHOLD

        if (isFlat != isRestingFlat) {
            isRestingFlat = isFlat
            if (isRestingFlat) {
                Log.i(TAG, "Phone placed flat – pausing movement")
                WebSocketManager.sendPauseMovement(true)
            } else {
                Log.i(TAG, "Phone picked up – resuming movement")
                WebSocketManager.sendPauseMovement(false)
            }
        }
    }
