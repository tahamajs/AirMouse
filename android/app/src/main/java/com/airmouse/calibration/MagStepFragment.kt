...
class MagStepFragment : Fragment(), SensorEventListener, CalibrationStepFragment {
    private lateinit var sensorManager: SensorManager; private var magSensor: Sensor? = null
    private val samples = mutableListOf<FloatArray>()
    private var sampleCount = 0; private val targetSamples = 200
    private var stepComplete = false
    ...
    override fun onSensorChanged(event: SensorEvent) {
        ...
        if (sampleCount >= targetSamples) {
            ...
            stepComplete = true
            (activity as? CalibrationActivity)?.onStepComplete()
            activity?.runOnUiThread { statusText.text = "Magnetometer calibrated!" }
        }
    }
