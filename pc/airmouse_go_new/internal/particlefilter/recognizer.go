...
// GestureEvent is emitted when a gesture is recognised.
...
// NewRecognizer creates a recogniser with default gestures.
...
// predefinedGestures returns a set of default gesture templates.
...
// AddMotion feeds a new movement delta to the recogniser.
...
// GetGesture returns the best matching gesture and confidence.
...
// matchTemplate performs DTW‑like matching against a template.
...
func (r *Recognizer) calculateAverageVelocity() float64 {
	...
	return total / float64(len(r.velocities))
}
...
// AddGestureTemplate adds a new gesture model.
...
// RemoveGestureTemplate removes a gesture by name.
...
// Reset clears all history and resets the filter.
...
// AddEventListener registers a callback for gesture events.
...
// GetAvailableGestures returns a list of gesture names.
func (r *Recognizer) GetAvailableGestures() []string {
	r.mu.RLock()
	defer r.mu.RUnlock()
	names := make([]string, len(r.gestures))
	for i, g := range r.gestures {
		names[i] = g.Name
	}
	return names
