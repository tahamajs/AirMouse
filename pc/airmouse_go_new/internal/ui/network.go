    ...
    tab.saveQrBtn = widget.NewButtonWithIcon("Save QR", theme.DownloadIcon(), func() {
        dialog.ShowFileSave(func(writer fyne.URIWriteCloser, err error) {
            if err == nil && writer != nil {
                defer writer.Close()
                data := fmt.Sprintf("airmouse://%s:%s?name=%s", tab.ipEntry.Text, tab.portEntry.Text, url.QueryEscape(cfg.ServerName))
                pngBytes, _ := qrcode.Encode(data, qrcode.High, 300)
                writer.Write(pngBytes)
                dialog.ShowInformation("Saved", "QR code saved successfully", fyne.CurrentApp().Driver().AllWindows()[0])
            }
