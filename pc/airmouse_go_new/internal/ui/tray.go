...
var (
	// IconData – main tray icon (16x16 PNG).
	IconData = []byte{
...
// Stop stops the tray application.
func (a *TrayApp) Stop() {
	if a.isRunning {
		a.stopServer()
	}
...
// ------------------------------------------------------------
// Server implementations (WebSocket, TCP, UDP)
// ------------------------------------------------------------
...
// ------------------------------------------------------------
// Mouse actions (platform‑specific – stubs)
// ------------------------------------------------------------
...
func (a *TrayApp) lockScreen() {
	...
	case "darwin":
		_ = exec.Command("osascript", "-e", `tell application "System Events" to keystroke "q" using {command down, control down}`).Start()
	case "linux":
		_ = exec.Command("gnome-screensaver-command", "-l").Start()
		_ = exec.Command("xdg-screensaver", "lock").Start()
	}
...
func (a *TrayApp) shutdownComputer() {
	switch runtime.GOOS {
	case "windows":
		_ = exec.Command("shutdown", "/s", "/t", "60").Start()
	case "darwin":
...
// ------------------------------------------------------------
// Settings windows (minimal stubs)
// ------------------------------------------------------------
...
func (a *TrayApp) showGesturesWindow() {
	...
	content := container.NewVBox(
		...
		widget.NewSeparator(),
		container.NewVBox(gestureWidgets...),
	)
...
func (a *TrayApp) showProximityWindow() {
	...
	nearSlider.OnChanged = func(v float64) {
		a.config.ProximityNearThreshold = v
	}

	farSlider := widget.NewSlider(2.0, 10.0)
	farSlider.Step = 0.2
	farSlider.Value = a.config.ProximityFarThreshold
	farSlider.OnChanged = func(v float64) {
		a.config.ProximityFarThreshold = v
	}
	...
	content := container.NewVBox(
		...
		widget.NewButton("Save", func() {
			_ = a.config.Save()
			dialog.ShowInformation("Saved", "Proximity settings saved.", w)
		}),
	)
...
func (a *TrayApp) showPersonalizationWindow() {
	...
	content := container.NewVBox(
		...
		widget.NewButton("Train Now", func() {
			dialog.ShowInformation("Training", "Training started...", w)
		}),
	)
