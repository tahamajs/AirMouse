...
func (m *LinuxMouse) Move(dx, dy float64) {
    if uinputFd == 0 {
        return // fallback: could use xdotool here
    }
...
func (m *LinuxMouse) Click(button string) {
    ...
    if button == "right" {
        ...
        btn = BTN_MIDDLE
        // middle click not counted separately; treat as click
        m.mu.Lock()
        m.clicks++
        m.mu.Unlock()
    } else {
    ...
    events := []inputEvent{
        {Type: EV_KEY, Code: uint16(btn), Value: 1}, // press
        {Type: EV_SYN, Code: 0, Value: 0},
        {Type: EV_KEY, Code: uint16(btn), Value: 0}, // release
        {Type: EV_SYN, Code: 0, Value: 0},
    }
    m.writeEvents(events)
}
...
func (m *LinuxMouse) writeEvents(events []inputEvent) {
    for _, ev := range events {
        // The size of inputEvent is 24 bytes on 64-bit systems.
