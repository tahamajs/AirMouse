...
// AISmoother uses an ONNX model to smooth mouse movements.
...
func (a *AISmoother) AddPoint(x, y float64) {
	...
	a.history = append(a.history, [4]float32{float32(x), float32(y), vx, vy})
...
func (a *AISmoother) applySmoothing(dx, dy float32) (float32, float32) {
	...
	if len(a.smoothingWindow) > 1 {
		...
		return sum / float32(len(a.smoothingWindow)), dy
	}
	return dx, dy
}
...
func (a *AISmoother) calculateConfidence(dx, dy float32) float64 {
	...
	conf := 0.5 + math.Min(0.5, magnitude/50.0)
	if magnitude > 100 {
		conf *= 0.7
	}
