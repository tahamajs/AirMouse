...
class AirMouseGUI:
    ...
    def _init_tray(self):
        try:
            ...
            self.log("ℹ️ pystray not available; system tray disabled")
            self.tray_icon = None
            return

        try:
            img = Image.new('RGB', (32, 32), color=self.accent)
            menu = pystray.Menu(
                pystray.MenuItem('Show', lambda _: self.root.deiconify()),
                pystray.MenuItem('Start Server', lambda _: self.start_servers()),
                pystray.MenuItem('Stop Server', lambda _: self.stop_servers()),
                pystray.MenuItem('Quit', lambda _: self._tray_quit())
            )
            icon = pystray.Icon('airmouse', img, 'Air Mouse', menu)
            self.tray_icon = icon
            ...
            self.tray_icon = None
            self.log(f'⚠️ Failed to initialize tray: {e}')
    ...
    def setup_ui(self):
        ...
        # keep a reference so we can enable/disable entries later
    ...
    def _rotate_logs(self):
        try:
            ...
            with open(fname, 'w', encoding='utf-8') as f:
                f.write(content)
            # keep last 500 lines in the UI
            lines = content.splitlines()[-500:]
            self.log_area.delete('1.0', tk.END)
            self.log_area.insert(tk.END, '\n'.join(lines) + '\n')
            self.log(f"🗄️ Rotated logs to {fname}")
        except Exception as e:
            self.log(f"⚠️ Failed to rotate logs: {e}")
