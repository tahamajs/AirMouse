...
func (s *Server) acceptLoop() {
	for s.running {
		...
		if err != nil {
			if s.running {
				utils.LogError("Accept error", "error", err)
			}
			continue
		}
...
func (s *Server) handleClient(conn net.Conn) {
	...
	for {
		...
		if err := json.Unmarshal([]byte(line), &msg); err != nil {
			utils.LogError("Invalid message", "error", err)
			continue
		}
...
func (s *Server) processMessage(client *Client, msg *Message) {
	...
	case "hello":
		...
		if err := json.Unmarshal(msg.Payload, &p); err == nil {
			...
			utils.LogInfo("Device identified", "id", client.ID, "name", client.Name)
			client.Conn.Write([]byte(`{"type":"welcome","payload":{"server":"AirMouse","version":"2.0"}}` + "\n"))
		}
	case "ping":
